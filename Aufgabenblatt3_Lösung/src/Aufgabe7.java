...
public class Aufgabe7 {
    ...
    private static void drawCirclesIterHand(CodeDraw myDrawObj, int y, int radius, int num){
        ...
        while (currentNum > 0) {
            ...
            // i < 1, i < 2, i < 4 ... so gelöst, um die Struktur von ChatGPT zu erhalten.
            for(int i = 0; i < Math.pow(2, num - currentNum); i++){
                myDrawObj.fillCircle(x, currentY + 2 * currentRadius * i, currentRadius);
            }
            currentNum--;
            currentRadius /= 2;
            currentY /= 2;
...
*/
        ...
        myDrawObjI40.setCanvasPositionX(50);
        myDrawObjI40.setCanvasPositionY(50);

        CodeDraw myDrawObjIHand = new CodeDraw(pixelWidth, pixelHeight);
        myDrawObjIHand.setTitle("Meine Version");
        myDrawObjIHand.setCanvasPositionX(600);
